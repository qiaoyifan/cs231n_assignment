...
def batchnorm_backward(dout, cache):
    ...
    d_running_mean = np.sum(dx_*-1/np.sqrt(running_var+eps),axis=0) + d_running_var*(-2*np.sum(x-running_mean,axis=0)/N)
    dx = dx_*(1.0/np.sqrt(running_var+eps)) + d_running_var*2*(x-running_mean)/N + d_running_mean/N
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    #切片[...,None]还没有搞清楚用法  2019.7.13
    sample_mean = np.mean(x,axis = 1)[...,None]
    #print('np.mean(x,axis = 1):shape: ')
    #print((np.mean(x,axis = 1)).shape)
    #print('sample_mean:shape: ')
    #print(sample_mean.shape)
    sample_var  = np.var(x,axis = 1)[...,None]
...
def layernorm_backward(dout, cache):
    ...
    #没有搞懂矩阵形状变换，自己凭运气调试对了    2019.7.13
    x,eps,gamma,beta,x_,sample_mean,sample_var = cache
    _,D = x.shape
    #print(D)
    dx_ = dout * gamma
    #print('dx_:shape: ')
    ...
    #print('sample_mean')
    #print(sample_mean)
    #print('np.sum(dx_*(x-sample_mean),axis = 1)')
    #print(np.sum(dx_*(x-sample_mean),axis = 1))
    #print('-0.5*(sample_var+eps)**(-3.0/2.0)')
    #print((-0.5*(sample_var+eps)**(-3.0/2.0)).shape)
    d_sample_var = (np.sum(dx_*(x-sample_mean),axis = 1))[...,None]*(-0.5*(sample_var+eps)**(-3.0/2.0))
    #print('d_sample_var')
    #print(d_sample_var)
    #print('d_sample_var:shape: ')
    #print(d_sample_var.shape)
    d_sample_mean = (np.sum(dx_*-1/np.sqrt(sample_var+eps),axis=1))[...,None] + d_sample_var*(-2*(np.sum(x-sample_mean,axis=1))[...,None]/D)
    #print('d_sample_mean:shape: ')
    #print(d_sample_mean.shape)
    dx = dx_*(1.0/np.sqrt(sample_var+eps)) + d_sample_var*2*(x-sample_mean)/D + d_sample_mean/D
    dgamma = np.sum(dout*x_, axis = 0)
    dbeta = np.sum(dout, axis = 0)
...
def max_pool_forward_naive(x, pool_param):
    ...
    for xi in range(H_):
        for yi in range(W_):
            x_mask = x[:,:,xi*stride:xi*stride+pool_height,yi*stride:yi*stride+pool_width]
            out[:,:,xi,yi] = np.max(x_mask, axis = (2,3))
...
def max_pool_backward_naive(dout, cache):
    ...
    for xi in range(H_):
        for yi in range(W_):
            ...
            dx[:,:,xi*stride:xi*stride+pool_height,yi*stride:yi*stride+pool_width] += flags * (dout[:,:,xi,yi])[:,:,None,None]
...
def spatial_batchnorm_backward(dout, cache):
    ...
    N, C, H, W = dout.shape
    dx_temp, dgamma, dbeta = batchnorm_backward(dout.transpose(0,2,3,1).reshape(N*H*W,C),cache)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    sample_var  = np.var(x,axis = 1)[...,None]
    x_ = (x - sample_mean) / np.sqrt(sample_var + eps)
    x_ = x_.reshape(N,C,H,W)
    out = gamma * x_ + beta
    cache = (x,eps,gamma,beta,x_,sample_mean,sample_var,G)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    #正确与否还未验证   2019.7.15
    x,eps,gamma,beta,x_,sample_mean,sample_var,G = cache
    N, C, H, W = x.shape
    dx_ = dout * gamma
    x = x.reshape(N*G, -1)
    ...
    d_sample_var = (np.sum(dx_*(x-sample_mean),axis = 1))[...,None]*(-0.5*(sample_var+eps)**(-3.0/2.0))
    d_sample_mean = (np.sum(dx_*-1/np.sqrt(sample_var+eps),axis=1))[...,None] + d_sample_var*(-2*(np.sum((x-sample_mean).reshape(N*G,-1),axis=1))[...,None]/C)
    dx = dx_.reshape(N*G,-1)*(1.0/np.sqrt(sample_var+eps)) + d_sample_var*2*(x-sample_mean)/C + d_sample_mean/C
    dgamma = np.sum(dout*x_, axis = (0,2,3))[None,...,None,None]
