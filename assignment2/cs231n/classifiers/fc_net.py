...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        da2 = relu_backward(dout, relu_cache2)   #此处relu_cache2 == a2
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for layer in range(self.num_layers):
            ...
            #需要判断一下 self.normalization == 'batchnorm',再将gamma参数初始化
            #否则后面可能会读到空的gamma值
            if self.normalization == 'batchnorm' or self.normalization == 'layernorm' and layer != self.num_layers-1:
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(self.num_layers):
            ...
            if self.normalization == 'layernorm' and layer != (self.num_layers-1):
                gammai,betai = self.params['gamma%d'%(layer+1)],self.params['beta%d'%(layer+1)]
                outi, ly_cachei = layernorm_forward(outi,gammai,betai,self.bn_params[layer])
                ly_cache_list.append(ly_cachei)
        ...
        for layer in range(self.num_layers,0,-1):
            ...
            #layer normalization
            if self.normalization == 'layernorm' and layer != self.num_layers:
                dout, dgamma, dbeta = layernorm_backward(dout,ly_cache_list[layer-1])
                grads['gamma%d'%(layer)] = dgamma
                grads['beta%d'%(layer)] = dbeta
            #backforward
            ...
            dout = dout.dot(self.params['W%d'%(layer)].T)
